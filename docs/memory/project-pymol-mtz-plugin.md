...
- **Global keyboard shortcuts are unreliable on macOS.** macOS intercepts modified arrow keys (Mission Control/Spaces) and many function keys before PyMOL sees them; `cmd.set_key` only accepts CTRL/ALT (not Command), and `[`/`]` won't bind ("Can't map regular letters"). So σ stepping is done via mouse-wheel-over-spinbox in the panel (0.02 σ). The `auto_mtz_keys` bindings (Alt-arrows) remain for other platforms. PyMOL's mouse binding (`cmd.button`) only maps to a fixed set of built-in view actions, so there is no scroll-to-contour on the 3D view itself.
